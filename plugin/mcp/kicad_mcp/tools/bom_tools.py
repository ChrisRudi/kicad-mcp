...
def register_bom_tools(mcp: FastMCP) -> None:
    ...
    async def analyze_bom(project_path: str, ctx: Context | None = None) -> dict[str, Any]:
        """Find every BOM file associated with a project, parse them, and report counts/categories/cost.

        Use this when the user wants BOM insight (component count by
        category, total parts, cost summary if currency fields exist).
        Don't open BOM CSVs by hand: this tool auto-discovers BOM files
        next to the project, classifies columns (CSV variations are
        rampant), and returns a normalised analysis.

        For just exporting a fresh BOM use ``export_bom_csv``. For the
        component list straight from the schematic (no BOM file needed)
        use ``list_schematic_components``.

        Args:
            project_path: Path to ``.kicad_pro``.

        Returns:
            ``{success, project_path, bom_files: {…}, component_summary:
            {total_unique_components, total_components, categories,
            total_cost?, currency?}}``.
        """
        ...
        for file_type, file_path in files.items():
            if "bom" in file_type.lower() or file_path.lower().endswith(".csv"):
                ...
                logger.debug(f"Found potential BOM file: {file_path}")
    ...
    async def export_bom_csv(project_path: str, ctx: Context | None = None) -> dict[str, Any]:
        """Export a fresh CSV BOM directly from a project's schematic via ``kicad-cli sch export bom``.

        Use this whenever the user wants an up-to-date BOM (overrides any
        stale on-disk BOM). Don't run ``kicad-cli`` manually — this tool
        resolves the schematic from the project, picks a sensible output
        path next to the schematic, and handles WSL→Windows path
        conversion.

        For analysis of an existing BOM use ``analyze_bom``.

        Args:
            project_path: Path to ``.kicad_pro``.

        Returns:
            ``{success, project_path, bom_path, columns}``.
        """
        project_path = to_local_path(project_path)
    ...
    try:
        if ext == '.csv':
            ...
            try:
                ...
                logger.warning(f"Failed to parse unknown file format: {file_path}: {e}")
    ...
    try:
        ...
        if cost_col:
            try:
                ...
                logger.warning("Failed to parse cost data: %s", e)
...
async def export_bom_with_python(schematic_file: str, output_dir: str, project_name: str, ctx: Context | None = None) -> dict[str, Any]:
    ...
    try:
        # Placeholder: exporting BOMs from schematic files via the KiCad
        # Python API is complex and the API surface is sparsely documented.
...
async def export_bom_with_cli(schematic_file: str, output_dir: str, project_name: str, ctx: Context | None = None) -> dict[str, Any]:
    ...
    if not kicad_cli:
        return {
            "success": False,
            "error": "KiCad CLI not found. Please install KiCad or set KICAD_CLI_PATH environment variable.",
            "schematic_file": schematic_file
        }
